				...
				<table id="table.data" style="margin-top:0px" class="table table-bordered table-data"
					...
					<thead>
						<tr>
							...
							<th style="width: 15%;">THUÊ TẠI</th>
							<th style="width: 15%;">ĐỊA CHỈ</th>
							<th style="width: 7%;">NGÀY THUÊ</th>
							<th style="width: 7%;">NGÀY TRẢ KẾ HOẠCH</th>
							<th style="width: 7%;">NGÀY TRẢ CHÍNH THỨC</th>
					...
					<tbody>
						...
						<c:forEach var="p" items="${lst}">
							<tr>
								...
								<td>${p.getBussiness_name()}</td>
								<td>${p.getBussiness_address()}</td>
								<td>${p.getRent_date()}</td>
								<td>${p.getPaid_1()}</td>
								<td>${p.getPaid_2()}</td>
								<td>${p.getStatus()}</td>
