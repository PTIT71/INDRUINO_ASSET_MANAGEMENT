			...
			<table class="table table-bordered table-search">
				<thead>
					<tr>
						...
						<input style="display: none" value="${InventoryID}" name="InventoryID"/>
...
<!-- 					</tr> -->
			...
			<div class="title-feature">
				<div class="text-right">
					...
					<button type="button" style="border-radius: 0" onclick="ToLink('InventorySessionInit')"
						class="btn btn-primary">
					...
					<c:forEach var="checking" items="${lstChecking}">
						<tr>
							...
							<c:if test="${checking.getStatus().trim() == '1'}">
								<td style="width:30px;text-align: right; color: green; background-color:green; font-weight: 700"></td>
							</c:if>
							<c:if test="${checking.getStatus().trim() == '2'}">
								<td style="width:30px;text-align: right; color: yellow; background-color:yellow; font-weight: 700"></td>
							</c:if>
							<c:if test="${checking.getStatus().trim() == '4'}">
								<td style="width:30px; text-align: right;background-color:red;  color: red; font-weight: 700"></td>
							</c:if>
						</tr>
					</c:forEach>
