...
<head>
<meta charset="UTF-8">
<title>Màn hình nghiệp vụ</title>
			...
			<div class="col-sm-12 general-setting shadow-sm p-3 mb-5 bg-gray">
				<form action="AssetManagementGeneral" method="POST">
					<table class="table table-bordered table-search">
						<thead>
							<tr>
								<td class="title">TÊN NGHIỆP VỤ</td>
								<td><input value="" name="major_name"></td>
								<td class="title">MÃ NGHIỆP VỤ</td>
								<td><input value="" name="major_code_name"></td>
								<td class="title">NỘI DUNG</td>
								<td><input value="" name="major_content"></td>
							</tr>
						</thead>
						<tbody>
						</tbody>
						...
						<tbody>
							...
							<c:forEach var="p" items="${listAssetSearch}">
								<tr>
									...
									<td><a style="text-decoration: none; font-weight: 700"
										href="AssetGeneralView?rfid_code=${p.RFID}">${p.RFID}</a></td>
									<td>${p.getName()}</td>
									<td><input type="text"
										style="display: inline-block; width: inherit; border: 0px; background-color: transparent;"
										readonly value="${p.getModel()}"></td>
									<td>${p.getSeries()}</td>
