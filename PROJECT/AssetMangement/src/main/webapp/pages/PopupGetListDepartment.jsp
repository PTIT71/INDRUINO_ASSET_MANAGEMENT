		...
		<table class="table table-bordered tablePopup"
			...
			<c:forEach var="p" items="${lst}">
				<tr>
					...
					<td style="width:80px;"><button style="width:100%; height:100%" onclick="return getvalue('${p.getDept_cd()}','${p.getDept_name()}')">CHỌN</button></td>
				</tr>
				</c:forEach>
			</tbody>
		</table>
...
<script type="text/javascript">
	function getvalue(code, name)
	{
		if (window.opener != null && !window.opener.closed) {
            var txtName = window.opener.document.getElementById("department_name");
            var txtcd = window.opener.document.getElementById("department_cd");
            txtName.value = name;
            txtcd.value = code;
        }
    window.close();
